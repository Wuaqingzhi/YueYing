...
**YueYing（月影）** 是一个 Android 网盘分享链接解析与高速下载应用。用户粘贴分享链接 → 浏览分享内容 → 取直链 → 分片并发下载到本地。

| 项 | 值 |
|---|---|
| 包名 | `com.yueying.app` |
| 源码根 | `app/src/main/kotlin/com/yueying/app` |
| 语言 | Kotlin |
...
```
app/src/main/kotlin/com/yueying/app/
├── MainActivity.kt              # 单 Activity 入口
├── YueYingApp.kt                   # Application，全局初始化
...
所有偏好读写集中在 `data/prefs/SettingsRepository.kt`（SharedPreferences 名 `yueying_settings`）。
...
（即 `/storage/emulated/0/Android/data/com.yueying.app/cache/download_tmp/{id}`）
